...
"""
    ...
    def test_compact_direct_change_uses_inline_fast_path(self):
        ...
        self.assertIn("references/approved-implementation-workflow.md", owners["skill"])
        self.assertIn("one short Plan, no duplicate Brief, and one initial Preflight", owners["requests"])
        ...
        self.assertIn("one regression per distinct changed behavior or failure mechanism", owners["approved"])
        adapter = " ".join((ROOT / "references" / "superpowers-adapter.md").read_text(encoding="utf-8").split())
        self.assertIn("strong reasoning capability makes inline execution preferable", adapter)
        self.assertIn("references/superpowers-adapter.md", owners["skill"])
    ...
    def test_strict_security_recovery_preserves_full_gates(self):
        ...
        self.assertIn("references/completion-contract.md", self.request_modes)
